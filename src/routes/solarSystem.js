...
let planets = [
    ...
    {id:"Saturn",   imagePath:'2k_saturn.jpg',              b:0.00025899,  c:-0.13434469, s:0.87320147,  f:38.35125000, a:9.54149883,  a0:-0.00003065, e:0.05550825, e0:-0.00032044, i:2.49424102, i0:0.00451969,  l:50.07571329,  l0:1222.11494724,  w:92.86136063,  w0:0.54179478, o:113.63998702, o0:-0.25015002,  orbitalEccentricity:0.052, mass:568,   orbitalPlaneAngle:2.49, rotationalAxisAngle:27,     color:'brown',      radius:58232,   dayInHours:10.7,      orbitDistance:886.5,    orbitTime:10747*24*3600},
...
function calcPosition(planet, T){ 
    ...
    return scaleOrbit(planet, position)
}
...
function animate() {
    ...
    if(controlTarget != _previousTarget || (cameraLock && simulationState)){
        camera.position.copy(targetPosition).addScaledVector(lockedCameraDirection, 5*(Math.max(minDistance,targetPlanet.scaledRadius)));
        _previousTarget = controlTarget;
    }
    //prevent camera from entering targetbody?
    ...
    if(simulationState == state.playing) {
        ...
        planets.forEach(planet => {
            ...
            planet.mesh.position.copy(planet.id != "Sun"? calcPosition(planet, simulationTime):center)
        });
    }
}
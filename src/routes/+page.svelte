<script>
	...
	let planets = {
		...
		Mercury: {
			...
			mass: 0.33,
			orbitalPlaneAngle: 7,
			rotationalAxisAngle: 0.1,
			color: 'gray',
			radius: 2439.7,
			dayInHours: 1407.6,
			orbitDistance: 36.8,
			orbitTime: 88 * 24 * 3600
		},
		Venus: {
			id: 'Venus',
			imagePath: '2k_venus_atmosphere.jpg',
			a0: 0.72332102,
			a: -0.00000026,
			e0: 0.00676399,
			e: -0.00005107,
			...
			mass: 4.97,
			orbitalPlaneAngle: 3.39,
			rotationalAxisAngle: 177,
			color: 'yellow',
			radius: 6051.8,
			dayInHours: 5832.5,
			orbitDistance: 67.2,
			orbitTime: 224.7 * 24 * 3600
		},
		Earth: {
			id: 'Earth',
			imagePath: '2k_earth_daymap.jpg',
			a0: 1.00000018,
			a: -0.00000003,
			e0: 0.01673163,
			e: -0.00003661,
			...
			mass: 5.97,
			orbitalPlaneAngle: 0,
			rotationalAxisAngle: 23,
			color: 'blue',
			radius: 6371,
			dayInHours: 23.9,
			orbitDistance: 93,
			orbitTime: 365.2 * 24 * 3600
		},
		Mars: {
			id: 'Mars',
			imagePath: '2k_mars.jpg',
			a0: 1.52371243,
			a: 0.00000097,
			e0: 0.09336511,
			e: 0.00009149,
			...
			mass: 0.642,
			orbitalPlaneAngle: 1.85,
			rotationalAxisAngle: 25,
			color: 'red',
			radius: 3389.5,
			dayInHours: 24.6,
			orbitDistance: 141.6,
			orbitTime: 687 * 24 * 3600
		},
		Jupiter: {
			id: 'Jupiter',
			imagePath: '2k_jupiter.jpg',
			b: -0.00012452,
			c: 0.0606406,
			s: -0.35635438,
			f: 38.35125,
			...
			mass: 1898,
			orbitalPlaneAngle: 1.31,
			rotationalAxisAngle: 3,
			color: 'orange',
			radius: 69911,
			dayInHours: 9.9,
			orbitDistance: 483.6,
			orbitTime: 4331 * 24 * 3600
		},
		Saturn: {
			id: 'Saturn',
			imagePath: '2k_saturn.jpg',
			ringImagePath: '2k_saturn_ring_alpha.png',
			b: 0.00025899,
			c: -0.13434469,
			s: 0.87320147,
			f: 38.35125,
			...
			mass: 568,
			orbitalPlaneAngle: 2.49,
			rotationalAxisAngle: 27,
			color: 'brown',
			radius: 58232,
			ringRadius: 282000,
			dayInHours: 10.7,
			orbitDistance: 886.5,
			orbitTime: 10747 * 24 * 3600
		},
		Uranus: {
			id: 'Uranus',
			imagePath: '2k_uranus.jpg',
			b: 0.00058331,
			c: -0.97731848,
			s: 0.17689245,
			f: 7.67025,
			...
			mass: 86.8,
			orbitalPlaneAngle: 0.77,
			rotationalAxisAngle: 98,
			color: 'lightblue',
			radius: 25362,
			dayInHours: 17.2,
			orbitDistance: 1783.7,
			orbitTime: 30589 * 24 * 3600
		},
		Neptune: {
			id: 'Neptune',
			imagePath: '2k_neptune.jpg',
			b: -0.00041348,
			c: 0.68346318,
			s: -0.10162547,
			f: 7.67025,
			...
			mass: 102,
			orbitalPlaneAngle: 1.77,
			rotationalAxisAngle: 30,
			color: 'darkblue',
			radius: 24622,
			dayInHours: 16.1,
			orbitDistance: 2795.2,
			orbitTime: 59800 * 24 * 3600
		}
	};
	...
	async function loadImages() {
		await Promise.all(
			planetList.map(async (planet) => {
				planet.image = await new THREE.TextureLoader().loadAsync(planet.imagePath);
				if (planet.id == 'Saturn')
					planet.ringImage = await new THREE.TextureLoader().loadAsync(planet.ringImagePath);
			})
		);
	}
	...
	function getSaturnRing(planet) {
		...
		for (let i = 0; i < pos.count; i++) {
			...
			geometry.attributes.uv.setXY(
				i,
				v3.length() < (outerRadius + planet.scaledRadius) / 2 ? 0 : 1,
				1
			);
		}
		const material = new THREE.MeshBasicMaterial({
	...
	const root = 2;
	export let radiusScalingFactor =
		maxDistance / 50 / (mileToKilometer * Math.pow(getPlanetByID('Sun').radius, 1 / root));
	...
	function rotatePlanetsAxis(planet) {
		...
		rotationalAxisQuart.setFromAxisAngle(
			new THREE.Vector3(1, 0, 0),
			2 * Math.PI * (planet.rotationalAxisAngle / 360)
		);
	...
	function adjustOrbitalPlane(planet) {
		...
		const x =
			(cosW * cosO - sinW * sinO * cosI) * planet.xPrime +
			(-sinW * cosO - cosW * sinO * cosI) * planet.yPrime;

		const y =
			(cosW * sinO + sinW * cosO * cosI) * planet.xPrime +
			(-sinW * sinO + cosW * cosO * cosI) * planet.yPrime;

		const z = sinW * sinI * planet.xPrime + cosW * sinI * planet.yPrime;
	...
	function scaleOrbit(planet, v) {
		...
		const minOrbitDistance =
			1.1 *
			(getPlanetByID('Sun').scaledRadius + getPlanetByID('Mercury').scaledRadius + 2 * minDistance);
		const maxOrbitDistance = maxDistance / 2;

		const scaledLength =
			minOrbitDistance + maxOrbitDistance * Math.pow((v.length() - minOrbit) / maxOrbit, 1 / 3);
	...
	function onKeyDown(e) 
	{
		switch (e.keyCode) {
			...
			case 38:
			case 40:
				if (periodSelect.matches(':focus-visible')){
					periodSelect.dispatchEvent(new KeyboardEvent("keypress", e));
				}
				else {
					// speedNumber.focus();
					// speedNumber.dispatchEvent(new KeyboardEvent("keypress", e));
					speedInput = Math.round(Math.min(1000, Math.max(0, speedInput+(e.keyCode==38?1:-1)*stepsize))*100)/100
				}
				break;
			case 37:
			case 39:
				// speedRange.focus();
				// speedRange.dispatchEvent(new KeyboardEvent("keypress", e));
				speedInput = Math.round(Math.min(1000, Math.max(0, speedInput+(e.keyCode==39?1:-1)*SlideSize)))
	...
	<div id="header" class:isHidden>
		<div class="header-control-div" style="border-width:thin;">
			<div class="time-div ctrl-div">
				<div class="ctrl-div">
					<button on:click={toggleState} style="min-width:30px"
						><i
							class={simulationState == state.playing
								? 'fa fa-solid fa-pause'
								: 'fa fa-solid fa-play'}
						></i></button
					>
					<button on:click={stopRestart} style="min-width:30px"
						><i
							class={simulationState != state.stopped
								? 'fa fa-solid fa-stop'
			...
			<div class="speed-div ctrl-div">
				...
				<div class="ctrl-div">
					<input id="speed-input"
						...
						max="1000"
						bind:value={speedInput}
						on:input={validateSpeedInput}
						bind:this={speedNumber}
						style="max-height:14.8px;"
					/>
					<select id="period" name="period" class="select-ctrl" bind:value={period} bind:this={periodSelect}>
